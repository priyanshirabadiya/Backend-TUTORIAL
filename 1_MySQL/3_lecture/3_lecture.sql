-- Order by , conditional operators , or - in - not in - between - not between - null - not null operators , Limit , distinct , count statement , group by , Like operator
...
-- 7) Limit opearator : limit give limit of range ex : limit 10 gives only first 10 records 
...
-- 8) distinct means one kind of unique value ex: if we have 5 country with same name then it will not show 5 country 5 times but it will show only one time that country 

select distinct country from customers;

-- 9) Count : count total of that perticular field

select count(customername) as Total_customers from customers; 

-- 10) Group by : group by and distinct is same but main difference is that we can count any fields data by using group by statement
